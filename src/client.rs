use crate::error::{FirestoreError, Error};
    ...
    pub async fn new<S: AsRef<str>>(
        ...
        let collection_path = {
            if collection_path.len() > 1 {
                let mut builder = db.parent_path(
                ...
                if collection_path.len() > 3 {
                    for tuple in collection_path[..collection_path.len() - 4]
                        .rchunks(2)
                    {
                        builder = builder.at(
                            tuple[0].as_ref(),
                            tuple[1].as_ref()).unwrap();
                    }
                }
                Some(builder.into())
            } else {
